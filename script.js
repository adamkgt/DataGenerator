...
document.addEventListener('DOMContentLoaded', ()=>{
  if(!fakerEN || !fakerPL || !fakerDE){ console.error('Faker (ESM) nie został poprawnie załadowany!'); return; }
  ...
  const renderTable = (cols, rows)=>{
    ...
    cols.forEach(c=>{ const th=document.createElement('th'); th.textContent=c.label; th.dataset.key=c.key; th.addEventListener('click', ()=>toggleSort(c.key)); tr.appendChild(th); });
    thead.appendChild(tr);
    tbody.innerHTML='';
    if(!rows.length){ const tr=document.createElement('tr'); const td=document.createElement('td'); td.colSpan=cols.length; td.className='empty'; td.textContent='Brak danych. Wygeneruj rekordy.'; tr.appendChild(td); tbody.appendChild(tr); }
    else{ rows.forEach(r=>{ const tr=document.createElement('tr'); cols.forEach(c=>{ const td=document.createElement('td'); td.textContent=r[c.key]??''; tr.appendChild(td); }); tbody.appendChild(tr); }); }
  ...
  $('#generate').addEventListener('click', ()=>{
    ...
    $('#count').value=n||0; if(!n){toast('Podaj liczbę rekordów (1–50).'); return;}
    setLocale($('#locale').value);
    const cols=currentColumns(); data=[];
    for(let i=0;i<n;i++){
      const name=genFullName(); const row={};
      cols.forEach(c=>{
    ...
    sortState={key:null, dir:1}; renderTable(cols,data); toast(`Wygenerowano ${n} rekordów`);
  });

  $('#copyBtn').addEventListener('click', async ()=>{
    if(!data.length){ toast('Brak danych do skopiowania.'); return; }
    const cols=currentColumns(); const header=cols.map(c=>c.label).join('\t');
    const text=[header,...data.map(r=>cols.map(c=>r[c.key]??'').join('\t'))].join('\n');
    await navigator.clipboard.writeText(text); toast('Skopiowano do schowka');
  });
